...
"offset ? rows fetch next 12 rows only;";
    ...
    public List<Product> search(String search, int index) {
        ...
        String sql = "with x as(select ROW_NUMBER() over (order by productImages desc) as r\n"
                + ",* from Product where productName like ?)\n"
                + "select productCode, productName, productStatus, productPrice, productOldPrice, productImages from x where r between ?*12-11 and ?*12";
        try {
            java.sql.Connection connection = new DBContext().getConnect();
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, "%" + search + "%");
